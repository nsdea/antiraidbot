...
COLOR = discord.Color(0x800040) # primary color for embeds
TESTING_MODE = socket.gethostname() in ['RTX3090', 'NB-SSA-Willich1'] # testing systems
PREFIX = 'ar!' # command prefix
...
async def on_command_error(ctx, error):
    ...
    error_messages = {
        ...
        commands.MissingRequiredArgument: f'Oops, I think you [*forgo**r*** 💀](https://i.redd.it/mc9ut2313b571.jpg) an argument, go check it using **`{PREFIX}help {ctx.message.content.replace(PREFIX, "").split()[0]}`**', # the f-string generates the help-command for the command
        commands.TooManyArguments: f'You gave too many arguments, use this command for help: **`{PREFIX}help {ctx.message.content.replace(PREFIX, "").split()[0]}`**', # the f-string generates the help-command for the command
        ...
        commands.BadArgument: f'There was a problem converting one of the argument\'s type, use this command for help: **`{PREFIX}help {ctx.message.content.replace(PREFIX, "").split()[0]}`**', # the f-string generates the help-command for the command
    }
...
async def commandinfo(ctx, name=None):
  if name:
    ...
    categories = {'🚨': 'Main commands', '📃': 'Information and help', '🔧': 'Tools and utilities', '🔒': 'Special', '🔩': 'Other'}
...
client.run(os.getenv('TOKEN')) # run bot with the token set in the .env file